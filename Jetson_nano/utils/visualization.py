...
class BBoxVisualization():
    ...
    def draw_bboxes(self, img, boxes, confs, clss):
        ...
        for bb, cf, cl in zip(boxes, confs, clss):
            ...
            Shcn = (y_max-y_min)*(x_max-x_min)
            if cl == 0 or cl ==1 :
               crop_image = img[y_min:y_max,x_min:x_max]
               crop_image = cv2.cvtColor(crop_image,cv2.COLOR_BGR2GRAY)
               height,width = img.shape[0],img.shape[1]
               revtal , dts = cv2.threshold(crop_image,140,255,cv2.THRESH_BINARY)
               canny_image = cv2.Canny(dts,150,255,apertureSize=7,L2gradient=True)
               half_image = dts[:int(0.55*height),:]
               cv2.imshow('test',half_image)
               Left = np.mean(half_image[30:half_image.shape[0]//2,:half_image.shape[1]//2])
               Right = np.mean(half_image[30:half_image.shape[0]//2,half_image.shape[1]//2:])
        ...
        return img,mid_point_x,Shcn,cl
